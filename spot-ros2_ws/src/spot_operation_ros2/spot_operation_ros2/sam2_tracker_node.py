        ...
        def inference(self, im, bboxes=None, points=None, labels=None, masks=None):
            ...
            try:
                return super().inference(
                    im, bboxes=bboxes, points=points, labels=labels, masks=masks
                )
            finally:
...
def _trim_sam2_memory(predictor, keep_frames: int = 6):
    ...
    def _trim_buckets(container, consolidated=None):
        for bucket in ("cond_frame_outputs", "non_cond_frame_outputs"):
            ...
            to_remove = (
                sorted_keys[:-keep_frames] if len(sorted_keys) > keep_frames else []
            )
    ...
    # Shared output dict
    _trim_buckets(
        state.get("output_dict", {}), state.get("consolidated_frame_inds", {})
    )

    # Per-object dicts — these hold the bulk of the per-frame GPU maskmem tensors and
    # are the actual leak if left untrimmed.
    for per_obj in (
        state.get("output_dict_per_obj", {}),
        state.get("temp_output_dict_per_obj", {}),
    ):
...
def _best_mask_from_results(results):
    ...
    if (
        r.boxes is not None
        and r.boxes.conf is not None
        and len(r.boxes.conf) == len(masks_np)
    ):
...
def _sam_prompt_masks(model, img_pil, point_xy, multimask_output=True):
    ...
    if not results:
        return np.empty(
            (0, img_np.shape[0], img_np.shape[1]), dtype=np.uint8
        ), np.array([])
    r = results[0]
    if r.masks is None or r.masks.data is None or len(r.masks.data) == 0:
        return np.empty(
...
class Sam2TrackerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            "hand_deroll_min_deg", 3.0
        )  # deadband; below this = no warp
        self.declare_parameter(
            "roll_reference_frame", "body"
        )  # gravity-up reference for the roll angle
        ...
        self.declare_parameter(
            "secondary_depth_topic_pattern", "/depth_registered/{cam}/image"
        )
        self.declare_parameter(
            "secondary_mask_topic_pattern", "/{cam}/segmentation_mask"
        )
        ...
        self.declare_parameter(
            "secondary_fov_enter_count", 5
        )  # consecutive seeds to arm init
        self.declare_parameter(
            "secondary_iou_radius_px", 80.0
        )  # radius (px) of the seed-neighbourhood disc
        ...
        self.declare_parameter(
            "secondary_max_mask_frac", 0.45
        )  # reject masks larger than this fraction of the image
        self.declare_parameter(
            "secondary_min_mask_area_px", 100
        )  # reject masks smaller than this (noise specks)
        self.declare_parameter(
            "secondary_seed_in_mask_tol_px", 6
        )  # seed must fall within this many px of the mask
        ...
        self.declare_parameter(
            "secondary_box_metric_clamp_m", [0.03, 0.8]
        )  # [min,max] object size sanity
        ...
        self.declare_parameter(
            "target_parent_frame", "world"
        )  # frame the box is published in
        self.declare_parameter("clear_box_topic", "/target_object/clear_box")
        self.declare_parameter(
            "clear_box_depth_extent_min_m", 0.04
        )  # floor for the along-view (z) thickness
        self.declare_parameter(
            "clear_box_depth_extent_max_m", 0.5
        )  # cap (reject depth outliers)
        self.declare_parameter(
            "secondary_warm_promote_frames", 3
        )  # frames of valid tracking → COLD→WARM
        self.declare_parameter(
            "secondary_warm_fail_max", 3
        )  # failed warm re-prompts → DEGRADED
        self.declare_parameter(
            "secondary_iou_min_overlap_warm", 0.15
        )  # relaxed IoU for warm re-prompts
        self.declare_parameter(
            "sam2_memory_reset_interval", 200
        )  # frames between memory resets
        ...
        self.declare_parameter(
            "secondary_seed_depth_radius_px", 8
        )  # NxN window = (2r+1)²
        ...
        self.publish_segmentation_mask = self.get_parameter(
            "publish_segmentation_mask"
        ).value
        self._hand_deroll_enabled = bool(
            self.get_parameter("hand_deroll_enabled").value
        )
        self._hand_deroll_min_deg = float(
            self.get_parameter("hand_deroll_min_deg").value
        )
        self._roll_reference_frame = str(
            self.get_parameter("roll_reference_frame").value
        )
        ...
        self.active_tracking_interval = self.get_parameter(
        ...
        secondary_rgb_topic_pattern = str(
            self.get_parameter("secondary_rgb_topic_pattern").value
        )
        secondary_depth_topic_pattern = str(
            self.get_parameter("secondary_depth_topic_pattern").value
        )
        secondary_mask_topic_pattern = str(
            self.get_parameter("secondary_mask_topic_pattern").value
        )
        ...
        self._secondary_tf_timeout_sec = float(
            max(0.0, self.get_parameter("secondary_tf_timeout_sec").value)
        )
        ...
        self._last_mask_np = (
            None  # last known mask; None = publish zeros (all background)
        )
        ...
        self._pending_seed_pixel_clock_t = (
            0.0  # node clock time (sec) when seed_pixel was queued
        )
        ...
        self._secondary_fov_timeout_sec = float(
            self.get_parameter("secondary_fov_timeout_sec").value
        )
        self._secondary_lost_guard_radius_px = int(
            self.get_parameter("secondary_lost_guard_radius_px").value
        )
        self._secondary_max_centroid_dist_px = float(
            self.get_parameter("secondary_max_centroid_dist_px").value
        )
        self._secondary_fov_enter_count = int(
            max(1, self.get_parameter("secondary_fov_enter_count").value)
        )
        self._sam2_memory_reset_interval = int(
            max(1, self.get_parameter("sam2_memory_reset_interval").value)
        )
        self._secondary_iou_radius_px = float(
            self.get_parameter("secondary_iou_radius_px").value
        )
        self._secondary_iou_min_overlap = float(
            self.get_parameter("secondary_iou_min_overlap").value
        )
        self._secondary_max_mask_frac = float(
            self.get_parameter("secondary_max_mask_frac").value
        )
        self._secondary_min_mask_area_px = int(
            max(1, self.get_parameter("secondary_min_mask_area_px").value)
        )
        self._secondary_seed_in_mask_tol_px = int(
            max(0, self.get_parameter("secondary_seed_in_mask_tol_px").value)
        )
        self._secondary_warm_promote_frames = int(
            max(1, self.get_parameter("secondary_warm_promote_frames").value)
        )
        self._secondary_warm_fail_max = int(
            max(1, self.get_parameter("secondary_warm_fail_max").value)
        )
        self._secondary_iou_min_overlap_warm = float(
            self.get_parameter("secondary_iou_min_overlap_warm").value
        )
        self._hand_reinit_speed_gate_m_s = float(
            self.get_parameter("hand_reinit_speed_gate_m_s").value
        )
        self._hand_reinit_speed_gate_timeout_s = float(
            self.get_parameter("hand_reinit_speed_gate_timeout_s").value
        )
        self._hand_seed_pixel_stamp_tolerance_s = float(
            self.get_parameter("hand_seed_pixel_stamp_tolerance_s").value
        )
        self._secondary_seed_pixel_stale_ms = float(
            self.get_parameter("secondary_seed_pixel_stale_ms").value
        )
        self._secondary_seed_depth_tol_m = float(
            self.get_parameter("secondary_seed_depth_tol_m").value
        )
        self._secondary_seed_depth_radius_px = int(
            max(1, self.get_parameter("secondary_seed_depth_radius_px").value)
        )
        self._hand_iou_radius_px = float(self.get_parameter("hand_iou_radius_px").value)
        self._hand_iou_min_overlap = float(
            self.get_parameter("hand_iou_min_overlap").value
        )
        self._hand_max_centroid_dist_px = float(
            self.get_parameter("hand_max_centroid_dist_px").value
        )
        self._secondary_box_scale = float(
            self.get_parameter("secondary_box_scale").value
        )
        ...
        self._clear_box_depth_min = float(
            self.get_parameter("clear_box_depth_extent_min_m").value
        )
        self._clear_box_depth_max = float(
            self.get_parameter("clear_box_depth_extent_max_m").value
        )
        _box_clamp = list(self.get_parameter("secondary_box_metric_clamp_m").value)
        self._secondary_box_metric_min = (
            float(_box_clamp[0]) if len(_box_clamp) > 0 else 0.03
        )
        self._secondary_box_metric_max = (
            float(_box_clamp[1]) if len(_box_clamp) > 1 else 0.8
        )
        ...
        self._tracking_state_pub = self.create_publisher(
            String, tracking_state_topic, 10
        )
        self._tracking_2d_pub = self.create_publisher(
            PointStamped, tracking_point_topic, 10
        )
        self._seed_3d_pub = self.create_publisher(PointStamped, "/tracking/seed_3d", 10)
        self._clear_box_pub = (
            self.create_publisher(
                Marker, str(self.get_parameter("clear_box_topic").value), 10
            )
            if self._clear_box_enabled
            else None
        )
        ...
        self._prompt_change_sub = self.create_subscription(
            String,
            "/vlm/prompt_change_id",
            self._prompt_change_cb,
            10,
            callback_group=self.io_cb_group,
        )
        # Geometry depth fallback: vision-frame object position reprojected to hand cam by tf_projection
        self._geometry_cam_pt: tuple = (
            None  # (x, y, z) in hand_cam frame, TF-derived (no depth sensor)
        )
        self.create_subscription(
            PointStamped,
            "/tracking/geometry_3d_in_cam",
            self._geometry_cam_pt_cb,
            10,
            callback_group=self.io_cb_group,
        )
        ...
        for cam in self._secondary_cameras:
            ...
            self.create_subscription(
                RosImage,
                secondary_rgb_topic,
                lambda msg, c=cam: self._secondary_rgb_cb(msg, c),
                10,
                callback_group=self.io_cb_group,
            )
            ...
            self.create_subscription(
                RosImage,
                secondary_depth_topic,
                lambda msg, c=cam: self._secondary_depth_cb(msg, c),
                10,
                callback_group=self.io_cb_group,
            )
            secondary_caminfo_topic = secondary_camera_info_topic_pattern.replace(
                "{cam}", cam
            )
            self.create_subscription(
                CameraInfo,
                secondary_caminfo_topic,
            ...
            self.create_subscription(
                PointStamped,
                f"/{cam}/tracking/seed_pixel",
                lambda msg, c=cam: self._secondary_seed_pixel_cb(msg, c),
                10,
                callback_group=self.io_cb_group,
            )
    ...
    def _coordinator_state_cb(self, msg: String):
        ...
        if state == "RELOCALIZING" and not self._snapshot_taken:
            ...
            for cam, st in self._secondary_cam_state.items():
                ...
                if st["needs_reinit"]:
                    self.get_logger().info(
                        f"[TRACKER] Pausing secondary cam {cam} during relocalization"
                    )
    ...
    def _save_snapshot_to_temp_dir(self):
        ...
        if (
            self._snapshot_rgb is None
            or self._snapshot_depth is None
            or self._snapshot_header is None
        ):
            return
        ...
        try:
            ...
            metadata = {
                ...
                "rgb_size": [
                    int(self._snapshot_rgb.width),
                    int(self._snapshot_rgb.height),
                ],
            ...
            self.get_logger().info(
                f"[TRACKER] Snapshot saved in temp dir: {self._snapshot_temp_dir}"
            )
        except Exception as exc:
            self.get_logger().warn(
                f"[TRACKER] Failed to save snapshot to temp dir: {exc}"
            )
    ...
    def _save_backproject_debug_image(self, img_pil, depth_map, pixel_uv, req_tag: str):
        ...
        try:
            ...
            depth_val_m = (
                float(depth[v, u])
                if np.isfinite(depth[v, u]) and depth[v, u] > 0
                else None
            )
            ...
            if finite.size > 0:
                ...
                sample_val = (
                    depth_val_m if depth_val_m is not None else float(np.median(finite))
                )
                ...
                if d_max <= d_min:
                    d_min, d_max = float(np.percentile(finite, 2)), float(
                        np.percentile(finite, 98)
                    )
                depth_clip = np.clip(depth, d_min, d_max)
                depth_norm = ((depth_clip - d_min) / (d_max - d_min) * 255.0).astype(
                    np.uint8
                )
            ...
            for panel, col in [
                (rgb_bgr, marker_color_rgb),
                (depth_color, marker_color_depth),
                (overlay, marker_color_ov),
            ]:
            ...
            for panel, caption in [
                (rgb_bgr, "RGB"),
                (depth_color, f"depth  [{d_min:.2f}-{d_max:.2f} m]"),
                (overlay, "overlay"),
            ]:
                cv2.putText(
                    panel,
            ...
            header_stamp = (
                self._snapshot_header.stamp
                if self._snapshot_header is not None
                else None
            )
            metadata = {
                "req_tag": req_tag,
                ...
                "stamp_sec": (
                    float(header_stamp.sec + header_stamp.nanosec * 1e-9)
                    if header_stamp
                    else None
                ),
                "frame_id": (
                    self._snapshot_header.frame_id
                    if self._snapshot_header is not None
                    else None
                ),
                "file": str(out_path),
            ...
            self.get_logger().warn(
                f"[TRACKER] Failed to save backproject debug image: {exc}"
            )
            return None
    ...
    def _maybe_log_rates(self):
        ...
        step_ms = (
            self._tracking_step_ms_sum / self._tracking_step_count
            if self._tracking_step_count
            else 0.0
        )
        self.get_logger().info(
    ...
    def _synced_image_cb(self, rgb_msg: RosImage, depth_msg: RosImage):
        ...
        try:
            ...
            self._frame_buffer.append(
                (stamp_sec, self.latest_rgb, self.latest_depth, depth_msg.header)
            )
        except Exception as e:
        ...
        if self._pending_seed_pixel_uv is not None:
            ...
            if self.tracking_active:
                ...
                if age > self._hand_reinit_speed_gate_timeout_s:
                    ...
                    stamp_sec
                    >= self._pending_seed_pixel_tnow
                    - self._hand_seed_pixel_stamp_tolerance_s
                ):
                    ...
                    if (
                        not self._hand_initialized_once
    ...
    def _update_display(self, img_pil=None):
        ...
        try:
            ...
            if self._hand_disp_bbox is not None and state in (
                "RELOCALIZING",
                "TRACKING",
            ):
            ...
            if state == "TRACKING":
                ...
                det = (
                    f"  [{self._hand_disp_label}  {self._hand_disp_conf:.2f}]"
                    if self._hand_disp_label
                    else ""
                )
            ...
            header = f"[hand]  {label}"
            cv2.putText(
                img_bgr, header, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 0), 3
            )
            cv2.putText(
                img_bgr,
                header,
            ...
            if self._hand_cam_speed is not None:
                ...
                speed_color = (
                    (0, 220, 0) if self._hand_cam_speed <= 0.05 else (0, 100, 255)
                )
                cv2.putText(
                    img_bgr,
                    speed_label,
    ...
    def _queue_debug_window(self, name: str, img_bgr, roll_angle=None) -> None:
        ...
        if roll_angle is not None:
            ...
            cv2.putText(
                img_bgr, txt, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 3
            )
            cv2.putText(
                img_bgr, txt, (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1
            )
        with self._gui_lock:
    ...
    def _measure_and_publish_clear_box(self, mask_np, depth_np, header):
        ...
        try:
            ...
            mask_d = (
                cv2.resize(mask_np, (dw, dh), interpolation=cv2.INTER_NEAREST)
                if (dw, dh) != (mw, mh)
                else mask_np
            )
            ...
            hz = 0.5 * float(
                np.clip(
                    z_hi - z_lo, self._clear_box_depth_min, self._clear_box_depth_max
                )
            )
            ...
            self.get_logger().warn(
                f"clear-box measure/publish failed: {exc}", throttle_duration_sec=2.0
            )
    ...
    def _apply_seed_command(self, seed: dict):
        ...
        if vlm_stamp is not None:
            img_pil, depth_for_seed, header = self._lookup_frame_at_stamp(
                float(vlm_stamp)
            )
        else:
        ...
        if img_pil is None:
            img_pil = (
                self._snapshot_rgb
                if self._snapshot_rgb is not None
                else self.latest_rgb
            )
            depth_for_seed = (
                self._snapshot_depth
                if self._snapshot_depth is not None
                else self.latest_depth
            )
            header = (
                self._snapshot_header
                if self._snapshot_header is not None
                else self.latest_depth_header
            )
            self.get_logger().warn(
                "[TRACKER] frame_stamp_sec not in seed or buffer miss — using snapshot/latest fallback"
            )
        ...
        for g in grasps_1000:
            ...
            grasp_points_uv.append(
                [max(0, min(orig_w - 1, gu)), max(0, min(orig_h - 1, gv))]
            )
        ...
        if sam_img_predictor is not None:
            ...
            if all_masks:
                ...
                if len(all_masks) > 1:
                    best_idx = int(
                        select_best_mask_by_bbox_iou(all_masks, bbox, img_pil.size)
                    )
                grasp_u, grasp_v = grasp_points_uv[
                    min(best_idx, len(grasp_points_uv) - 1)
                ]
        ...
        debug_path = self._save_backproject_debug_image(
            img_pil, depth_for_seed, (grasp_u, grasp_v), backproject_tag
        )
        ...
        if (
            not hasattr(self, "_reference_seed_depth")
            or self._reference_seed_depth is None
        ):
            self._reference_seed_depth = z
        else:
            ratio = (
                z / self._reference_seed_depth
                if self._reference_seed_depth > 0
                else 999.0
            )
            if ratio > 2.5 or ratio < 0.4:
                ...
                raise RuntimeError(
                    f"Seed depth {z:.2f}m too far from reference {self._reference_seed_depth:.2f}m"
                )
    ...
    def _do_video_predictor_init(self, u: int, v: int, img_pil, depth_np, header):
        ...
        if roll_angle != 0.0:
            ...
            self._queue_debug_window(
                "Hand Upright (de-rolled)",
                cv2.cvtColor(init_img_np, cv2.COLOR_RGB2BGR),
        ...
        if best_tracked_mask is None:
            self.get_logger().error(
                "Video predictor init produced no mask from reprojected pixel"
            )
            return
    ...
    def _run_tracking_step_2d_body(self, now: float):
        ...
        if point_cam is None:
            ...
            z, ok = self._get_valid_depth(
                self.latest_depth, grasp_u, grasp_v, radius=10
            )
            if not ok:
    ...
    def _depth_msg_to_np(self, msg):
        ...
        try:
            ...
            self.get_logger().warn(
                f"depth conversion error: {e}", throttle_duration_sec=2.0
            )
            return None
    ...
    def _secondary_seed_pixel_cb(self, msg: PointStamped, cam: str):
        ...
        st["consecutive_seed_count"] = (
            self._secondary_fov_enter_count
        )  # bypass hysteresis
        ...
        try:
            ...
            if state == "TRACKING" and mask_np is not None:
                ...
                if centroid_uv and expected_uv:
                    dist = (
                        (centroid_uv[0] - expected_uv[0]) ** 2
                        + (centroid_uv[1] - expected_uv[1]) ** 2
                    ) ** 0.5
    ...
    def _run_secondary_tracking_step(self, cam: str):
        ...
        if new_frame and frame_hdr is not None:
            ...
            if frame_seed is not None:
                ...
                st["init_uv"] = (u_s, v_s)
                st["init_uv_stamp"] = (
                    frame_hdr.stamp.sec + frame_hdr.stamp.nanosec * 1e-9
                )
        ...
        if (
            not st["tracking_initialized"]
            and not st["needs_reinit"]
            and not is_warm_with_predictor
        ):
        ...
        if st["video_predictor"] is None:
            if not SAM2_AVAILABLE:
                return
            try:
                st["video_predictor"] = SAM2ROSVideoPredictor(
                    overrides={
                        "conf": 0.01,
                        "task": "segment",
                        "mode": "predict",
        ...
        if init_needed and st["init_uv"] is not None:
            ...
            if (not is_warm) and self._hand_object_metric_size is not None:
                ...
                if z_sec is not None and z_sec > 0.0 and intr is not None:
                    ...
                    half_w = (
                        0.5
                        * self._hand_object_metric_size
                        * self._secondary_box_scale
                        * fx_s
                        / z_sec
                    )
                    half_h = (
            ...
            if mask is not None:
                ...
                seed_patch = mask_u8[
                    max(0, v_c - tol) : v_c + tol + 1, max(0, u_c - tol) : u_c + tol + 1
                ]
                ...
                if (not is_warm) and expected_depth is not None:
                    ...
                    if depth_msg is not None:
                        ...
                        if depth_np is not None:
                            z_mask, ok_m = self._median_depth_under_mask(
                                depth_np, mask_u8
                            )
                            if (not ok_m) or abs(
                                z_mask - expected_depth
                            ) > self._secondary_seed_depth_tol_m:
                                mask_depth_bad = True
                            ...
                            z_seed, ok_s = self._get_valid_depth(
                                depth_np, du, dv, radius=6
                            )
                            self.get_logger().warn(
                ...
                if mask_area < self._secondary_min_mask_area_px:
                    ...
                    not skip_area_check
                ) and mask_frac > self._secondary_max_mask_frac:
                    self.get_logger().warn(
                ...
                if not st["tracking_initialized"]:
                    ...
                    self.get_logger().info(
                        ...
                        + (
                            ...
                            if (z_mask is not None and expected_depth is not None)
                            else ""
                        ),
                    )
                ...
                try:
                    ...
                    dbg_arr = cv2.addWeighted(dbg_arr, 1.0, green_overlay, 0.45, 0)
                    cv2.drawMarker(
                        dbg_arr, (u, v), (255, 0, 0), cv2.MARKER_CROSS, 20, 2
                    )
                    ...
                    dbg_path = (
                        self._snapshot_temp_dir
                        / f"sam2_secondary_init_{cam}_{self._snapshot_run_idx:04d}.png"
                    )
                    cv2.imwrite(str(dbg_path), cv2.cvtColor(dbg_arr, cv2.COLOR_RGB2BGR))
                    self.get_logger().info(
                        f"[{cam}] SAM2 secondary init debug saved: {dbg_path}"
                    )
                except Exception as _exc:
                    self.get_logger().warn(
                        f"[{cam}] SAM2 secondary init debug save failed: {_exc}"
                    )
            else:
                st["_disp_state"] = "LOST"
                st["_disp_mask"] = None
        ...
        # own without waiting for a new seed prompt.
            ...
            if mask is not None and score > 0.0:
                ...
                if m_cv["m00"] != 0:
                    centroid = (
                        int(m_cv["m10"] / m_cv["m00"]),
                        int(m_cv["m01"] / m_cv["m00"]),
                    )
                ...
                if is_warm:
                    ...
                    if (
                        mask_area < self._secondary_min_mask_area_px
                ...
                if not consistent:
                    st["lost_streak"] += 1
                    self.get_logger().warn(
                        f"[{cam}] {reject_reason}", throttle_duration_sec=1.0
                    )
                ...
                if consistent:
                    ...
                    if st.get("lifecycle") == "COLD":
                        st["warm_tracked_frames"] += 1
                        if (
                            st["warm_tracked_frames"]
                            >= self._secondary_warm_promote_frames
                        ):
                            st["lifecycle"] = "WARM"
                            self.get_logger().info(
                    ...
                    if (
                        st["tracking_frame_count"] % self._sam2_memory_reset_interval
                        == 0
                    ):
                        _trim_sam2_memory(st["video_predictor"], keep_frames=6)
                        self.get_logger().info(
                ...
                if st["lost_streak"] >= self.tracking_lost_confirm_frames:
                    ...
                    st["consecutive_seed_count"] = 0  # re-arm from scratch
                    suffix = (
                        " (WARM — next seed re-prompts)"
                        if st.get("lifecycle") == "WARM"
                        else ""
                    )
                    self.get_logger().warn(
                        f"[{cam}] Tracking lost — no mask for too long{suffix}"
                    )
                st["_disp_state"] = "LOST"
                st["_disp_mask"] = None
                if self.visualize:
...
def main(args=None):
    ...
    if node.visualize:
        ...
        exec_thread = threading.Thread(
            target=executor.spin, name="sam2_executor", daemon=True
        )
        exec_thread.start()
        try:
            while rclpy.ok():
